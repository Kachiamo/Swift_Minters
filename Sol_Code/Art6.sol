...
contract NFTMarketplace is ERC721, Ownable {
    ...
    function unlistNFT(uint256 _tokenId) external onlyNFTOwner(_tokenId) {
        require(nfts[_tokenId].listed, "NFT not listed");
         require(block.timestamp >= listingTimestamps[_tokenId],"Listing is still locked");
    ...
    function listingNFTForSale(uint256 _tokenId, uint256 _price, uint256 _lockduration) external onlyNFTOwner(_tokenId){
        require(!nfts[_tokenId].listed,"NFT already listed");
        nfts[_tokenId].price =_price;
        nfts[_tokenId].listed = true;
        listingTimestamps[_tokenId]= block.timestamp + _lockduration; // Lock duration in seconds
        emit NFTListed(_tokenId,_price);
    }
